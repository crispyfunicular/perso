...
seed(0) # is used to initialized the pseudo random number generator, change it to have a different run
...
def smallest_1(l, verbose=False): # get the smallest element of a list
    ...
    for i in range(len(l)-1): # we iterate over the list but do not need to check the first element
        count += 1
        if l[i+1] < m: # if the ith element is actually smaller than m, remember it
            m = l[i+1]

    if verbose:
        print('smallest element :', m, 'of', l, 'number of comparisons :', count, sep='\t')
...
m, ncomp = smallest_1(l.copy()) # copy l so that we keep our original list for other sorting theynanigans (it's inclusive)
print(m, ncomp)
...
sorted_l, ncomp = sort_1(l.copy(), verbose=True) # copy l so that we keep our original list for other sorting shenanigans
...
for _ in range(10):
    random_list = [randint(0, 1000) for _ in range(randint(10, 30))] # a random list of random length
...
for _ in range(10):
    random_list = [randint(0, 1000) for _ in range(randint(10, 30))] # a random list of random length
    sorted_l, ncomp = sort_2(random_list) 
