...
def dichotomy(el, lst):
    ...
    while low < high - 1: # if low == high - 1, then we have reached two consecutive number and have rejected everything outside, so we are close to the end
        mid = (low + high) // 2 # remember this means euclidean division, it's because if (low + high) is odd (5 for example), the middle is not a whole number : 2.5
