...
print('x :', x, len(str(x)), str(log10(x))[:6], bin(x)[2:], len(bin(x))-2, str(log2(x))[:6], sep='\t')
print('y :', y, len(str(y)), str(log10(y))[:6], bin(y)[2:], len(bin(y))-2, str(log2(y))[:6], sep='\t')
...
bx = bin(x)[2:]  # get the binary representation of x. the two first characters are a way to indicate that it's a number in binary.
...
for i in [0, 1, 2, 3, 4, 8, 16, 1347, 8521, 163245]:
    print(i, bin(i)[2:], binary_1(i), bin(i)[2:] == binary_1(i), sep='\t') # If your function works you should have True at the end of each line





def binary_2(x): # now we do the same but in the other direction, without computing the columns
...
print('3.2. Exercices')
for i in [0, 1, 2, 3, 4, 8, 16, 1347, 8521, 163245]:
    print(i, bin(i)[2:], binary_2(i), bin(i)[2:] == binary_2(i), sep='\t') # If your function works you should have True at the end of each line
...
print()
print('4. Exercices')
print(0, '0', base_5(0), '0' == base_5(0), sep='\t') # If your function works you should have True at the end of each line
